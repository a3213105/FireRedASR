...
class FireRedAsrAed_ov :
    def __init__(self, args, ov_core, model_path, infer_type = "f32"):
        self.INF = 1e10
        self.sos_id = args.sos_id
        self.eos_id = args.eos_id
        ...
        try :
            ...
            self.ov_encoder_model = ov_core.compile_model(self.ov_encoder_path, 'CPU', ov_config)
            self.ov_decoder0_model = ov_core.compile_model(self.ov_decoder0_path, 'CPU', ov_config)
            self.ov_decoder1_model = ov_core.compile_model(self.ov_decoder1_path, 'CPU', ov_config)
            self.using_ov = True
    ...
    def convert_ov_model(self, feats, lengths, beam_size, nbest, decode_max_len,
        ...
        if not self.ov_encoder_path.exists() :
            ...
            save_model(ov_model, self.ov_encoder_path)
        ...
        #     ov.save_model(ov_model, self.ov_decoder_path)
        #     del ov_model
        #     cleanup_torchscript_cache()
        ...
        if not self.ov_decoder0_path.exists() :
            ...
            ov.save_model(ov_model, self.ov_decoder0_path)
            del ov_model
            cleanup_torchscript_cache()
        ...
        if not self.ov_decoder1_path.exists() :
            ...
            ov.save_model(ov_model, self.ov_decoder1_path)
            del ov_model
            cleanup_torchscript_cache()
